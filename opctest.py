...
def mainloop(cyctime = 1, faultcy = 5, statcy = 10, nostop = True):
    c = 0 # init cycle counter
...
def runchk(run): # loop when system is idle
    opc.connect(opc_server)
    run, state, time = opc.read(runtag)
    opc.close()
    if not run:
        mailalert(body = 'Run has finished. \n')
    return(run)
